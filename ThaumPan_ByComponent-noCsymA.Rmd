...
Adjust thaumtab annotation to exclude CsymA
```{r}
...
for(i in 1:length(categories)) {
  result = c(as.character(categories[i]), sum(str_detect(thaumtab_noCsym$V3, categories[i]), na.rm=TRUE))
  pancounts = rbind(pancounts, result, stringsAsFactors=FALSE)
}

pancounts = rbind(pancounts, c("NA", sum(is.na(thaumtab_noCsym$V3))))
...
ggsave('rplots/cogcatvsnumber_noCsymA.pdf', plot = last_plot(), device = NULL, path = NULL,
...
ggsave('rplots/cogcatvsnumber_noCsymA-noPC.pdf', plot = last_plot(), device = NULL, path = NULL,
  scale = 1, units = "in", width= 20, height= 10, dpi = 300, limitsize = TRUE)
...
strictCoreCounts = getcounts(nStrictCore, categories, thaumtab_noCsym)
strictAccCounts = getcounts(nStrictAcc, categories, thaumtab_noCsym)
...
softCoreCounts = getcounts(nSoftCore, categories, thaumtab_noCsym)
softAccCounts = getcounts(nSoftAcc, categories, thaumtab_noCsym)
...
ggsave('rplots/coreaccessorystack_noCsymA_strict.pdf', plot = last_plot(), device = NULL, path = NULL,
  scale = 1, units = "in", width= 20 , height= 10, dpi = 300, limitsize = TRUE)
...
ggsave('rplots/coreaccessorystack_noCsymA_soft.pdf', plot = last_plot(), device = NULL, path = NULL,
  scale = 1, units = "in", width= 20 , height= 10, dpi = 300, limitsize = TRUE)
